...
- sonst pro enabled Regel: **zuerst die harten Gates** prüfen — **nur** `match_label` + `match_priority_min` (eigene kleine Gate-Methode, **nicht** das alte `matches()` wiederverwenden! `matches()` gated auch hart auf `match_sender_key`/`match_from_local` → sonst wäre die Domain-/Fuzzy-Logik des `MatchScorer` tot, weil der Absender vorher exakt gefiltert würde). `match_sender_key`/`match_from_local`/`match_subject_regex` gehen **ausschließlich** in den `MatchScorer` als gewichtete Features (kein Doppel-Gating von `subject_regex`). Dann `score = matchScorer->score($rule, $mailFeatures)`. Im `match_mode` ∈ {llm,hybrid} **und** `$wasCacheHit === false` **und** `band==suggest` **und** Batch-Budget übrig → `ruleMatch->scoreMatch()` als Verfeinerung (`null` → deterministischen Score behalten). Dann: `auto` → `applySetFieldsOrthogonal()` (orthogonal, Sticky-Schutz unverändert) · `suggest` → Vorschlag anlegen (s. (d)), **kein** Score-Write · `ignore` → nichts.

(c) **Cache-Hit-Info durchreichen (heute NICHT vorhanden!):** `scoreBatch()` baut `$cacheHits` (Liste `['mail'=>…,'score_index'=>…]`), reicht sie aber NUR an `actionOwner->resolveForCacheHits()`. Daher: `enrichScoresWithSender(..., array $cacheHitMailIds)` um einen Parameter erweitern (Set der `mail_id`s, die Cache-Hits waren — aus `$cacheHits` abgeleitet) und pro Row `apply(..., $bucket, in_array($mid, $cacheHitMailIds, true))` aufrufen. `apply(...)` bekommt `bool $wasCacheHit = false` als letzten Parameter (Default deckt Alt-Aufrufer ab). **Zweiter Aufrufer:** `MailScoringService::applyOverrideToExistingScore()` (Click-Time via MailController) ruft `apply()` ebenfalls — ihm `$wasCacheHit=true` übergeben, damit Click-Time deterministisch bleibt (kein LLM-Match pro Klick/Kosten-Überraschung).
...
(a) **`origin_correction_id` durchreichen:** `inferScoreRule`/`inferTopicRule`/`inferAllFromCorrection` haben heute KEINEN `correctionId`-Parameter — einen ergänzen und vom Caller `MailController::correctScore()` (kennt die `mail_score_corrections.id`) durchreichen. **Achtung Re-Korrektur:** `CorrectionRepository::record()` macht `ON DUPLICATE KEY UPDATE` und liefert dann eine *frische, weggeworfene* UUID, die NICHT der gespeicherten Row-id entspricht → als Marker unzuverlässig. Der zuverlässige UNIQUE-Key ist **`uq_correction_per_mail (tenant_id, mail_id)`** (Migration 0007): die bestehende id via `SELECT id FROM mail_score_corrections WHERE tenant_id=? AND mail_id=?` auflösen (**nicht** über `rule_inference_hash` — das ist NULLABLE und wird von `record()` nie befüllt) und DIESE als `origin_correction_id` nutzen. In den Erzeugungspfaden vor dem Anlegen `findUserDerivedSlot($tenant,$user,$senderKey,$field)`; existiert ein Slot → `updateFields()` statt `create()`. Dabei die bestehenden `hasSimilarPriorityRule`/`hasSimilarTopicRule`-**Skip**-Branches in `inferAllFromCorrection` durch diesen Slot-Lookup+Update **ersetzen** (sonst wird bei vorhandener Regel weiter mit `duplicate_rule_exists` geskippt statt aktualisiert). `create()` immer mit `origin_correction_id` + `source='ki_inferred'`. Confidence → Match-Breite: hoch → `match_sender_key` (Domain erlaubt); niedrig → zusätzlich `match_from_local` (enger).
