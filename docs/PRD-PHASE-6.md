...
| **6f — Auto-Reply-Drafts** | Worker-Hintergrund-Job erzeugt für `action_owner=user AND action_required=1 AND priority>=floor` Mails einen Opus-Draft. Nichts wird automatisch gesendet — User sieht Draft im „Diese Mail"-Tab und kann übernehmen/anpassen/verwerfen. | 2-3 Tage (DA-Runden 1+2) | • Schema (Migration 0027): `reply_drafts.created_by` ENUM(user,auto), `stale_at` DATETIME, `dismissed_at` DATETIME, `user_id` (für DSGVO-Filter), `conversation_id` für Stale-Match. Plus Settings `autoreply_enabled` (bool, Default 0), `autoreply_enabled_at` (string, ISO-UTC, Default NULL), `autoreply_priority_floor` (int, Default 4), `autoreply_only_owner_user` (bool, Default true). <br>• Worker-Tick alle 5 min sucht Kandidaten: `action_owner=user AND action_required=1 AND priority>=autoreply_priority_floor AND cleared_at IS NULL AND received_at >= autoreply_enabled_at AND keine non-dismissed Draft` <br>• Quota via `UsageCounterRepository::incrementOrFail('auto_reply', autoreply_max_per_day)` — Infrastruktur aus 6g <br>• Cost-Cap via BudgetService prüft Opus-Token-Budget vor Call <br>• **DA-R1 Finding 1 (High) — Sent-Folder-Blindheit:** Pre-Generation Graph-Lookup auf `conversations/{conversation_id}/messages?$top=1&$orderby=receivedDateTime desc`. Wenn letzte Message `from=me` ODER nach `mail.received_at` gesendet wurde: skip + setze `stale_at=NOW()` auf existierende Drafts dieser Konversation. <br>• **DA-R2 Finding 1 (High) — Identity-Set für `from=me`:** Match gegen `users.aliases[] ∪ mailboxes.email_address[] für tenant_id` (Sprint-6a Multi-Alias). Test pinnt Alias-Match. <br>• Stale-Detection bei neuer Inbox-Mail: bei selber `conversation_id` → markiert existierende Drafts `stale_at=NOW()` <br>• **DA-R1 Finding 2 (High) — Cold-Start-Spike:** `autoreply_enabled` Default **0** (opt-in). Beim Settings-Toggle-On wird `autoreply_enabled_at` auf jetzt-UTC gesetzt. Worker-Filter `received_at >= autoreply_enabled_at` schließt Backlog aus, bis Marc explizit „Backlog inkludieren"-Button klickt (setzt enabled_at=NULL einmalig, generiert einen einzelnen Sweep-Tick mit Quota-Cap intact). <br>• `autoreply_priority_floor` (Default 4), `autoreply_only_owner_user` (Default true) <br>• ReplyDraftService::generateAuto(...) — wiederverwendet existing draft()-Methode aber mit `user_instruction=null` und `created_by='auto'` <br>• PII-Redaction läuft wie in Phase-B-Pfad (subject/body via redactMail). **DA-R2 Finding 3 (Medium):** Redactor wird pro Call mit `(tenant_id, user_id)`-Scope instantiiert — nicht als Konstruktor-Singleton — damit `redaction_rules`-User-Patterns greifen. <br>• **DA-R1 Finding 3 (Medium) — Output-PII:** Nach `extractText($resp)` und VOR `drafts->create()` ein `redactor->redact($draftText)`-Pass — fängt halluzinierte IBANs/CC im KI-Output. Test pinnt das mit gemocktem Claude-Response der IBAN enthält. <br>• **DA-R1 Finding 4 (Medium) — FYI-Filter + Re-Generate:** Worker-Pre-Filter skippt Opus-Call bei `subject =~ /best[äa]tigung\|confirmation\|receipt\|order #/i` ODER `body_text < 200 chars`. Regex aus `system_settings.autoreply_skip_subject_regex` (admin-editable). Plus UI-Button „Neuen Entwurf" wenn `dismissed_at IS NOT NULL` — ruft existing on-demand `ReplyDraftService::draft()`, zählt NICHT gegen auto_reply-Quota (User-initiated). <br>• **DA-R2 Finding 2 (High) — Observability:** Jeder Skip-Pfad inkrementiert `usage_counters.kind` mit prefix `auto_reply_skip_<reason>` (`sent_match`, `before_enabled_at`, `fyi_filter`, `quota_exceeded`, `budget_exceeded`, `no_conversation_id`). Plus Worker-Log-Entry `auto_reply.skipped` mit Grund. Admin-Panel-Widget zeigt letzte 24h Skip-Reasons. <br>• UI: Draft-Box im „Diese Mail"-Tab über Klassifikation, Buttons „In Outlook öffnen", „Verwerfen", „Neuen Entwurf" (nur wenn dismissed). Heute-Karten zeigen 📝-Badge wenn non-dismissed Draft vorhanden. <br>• `reply_drafts` schon in /me/export + /me/delete; neue Spalten transparent abgedeckt durch DSGVO-Coverage-Test |
